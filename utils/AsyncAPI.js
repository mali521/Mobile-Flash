...
export function getDecks() {
  ...
  return AsyncStorage.getAllKeys().then((keys) => {
    return AsyncStorage.multiGet(keys).then((allDecks) => {
...
export function saveDeckTitle(title) {
  return getDeck(title).then((deck) => {
    ...
    return AsyncStorage.setItem(title, JSON.stringify(newDeck)).then((err) => {
...
export function addCardToDeck(title, card) {
  return getDeck(title).then((deck) => {
    ...
    return AsyncStorage.mergeItem(title, JSON.stringify(deckObj)).then(
      (err) => {
    ...
    if (Constants.isDevice && status === "granted") {
      ...
      Notifications.cancelAllScheduledNotificationsAsync().then((result) => {
        ...
        const handleNotification = ({ notificationId }) => {
          this.chkForQuizAttemptedToday().then((result) => {
        ...
        const localNotification = {
          ...
          body: `Hey!, Is everything fine today? You haven't attemeted any quiz today!! Today is ${currentDate.getDate()}/${
            currentDate.getMonth() + 1
          }/${currentDate.getFullYear()} `,
          ios: { sound: true },
          android: {
